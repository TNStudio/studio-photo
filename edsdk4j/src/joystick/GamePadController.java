...
/**
 * 
 * @author modified by Martin
 *
 */
public class GamePadController{

	/*Declarations*/
	public static final int NUM_BUTTONS = 10; 					//number of buttons
	//change the number of button if needed

	public static final int NUM_COMPASS_DIRS = 9;				//number of positions

	public static final int NW = 0;								//directions
	...
	private Controller controller;								//net.java.games.input object

	private Component[] comps;									//tab of the game pad component

	private int xAxisIdx, yAxisIdx, rxAxisIdx, ryAxisIdx, zAxisIdx; //Index of the axis
	...
	public GamePadController() throws NoJoystickException{
		
		System.out.println("Try to reference all the controllers...");
		ControllerEnvironment ce = ControllerEnvironment.getDefaultEnvironment();	//pick up the controllers
		Controller[] cs = ce.getControllers(); 										//new tab of controllers
		if (cs.length == 0) {
			throw new NoJoystickException(); 						//if no hid device (mouse, keyboard, gamepad)
		...
		System.out.println("try to identify gamepad in the controllers list...");
		controller = findGamePad(cs); 												//internal function to find a gamepad, see below
		...
		findCompIndices(controller); //internal function to find the indices of the components, see below
		findRumblers(controller);  // internal function to find the rumblers, see below
	}

	public Controller findGamePad(Controller[] cs) throws NoJoystickException {
		Controller.Type type;
		int i = 0;
		while(i < cs.length) {														// get the type of each controller 
			type = cs[i].getType();
			if ((type == Controller.Type.GAMEPAD) || (type == Controller.Type.STICK))
				break; 																//take the first game pad found for the listener
			i++;
		}

		if (i == cs.length) {
			throw new NoJoystickException();
	...
	private void findCompIndices(Controller controller) {
     ...
     button components of the controller.
		 */
		comps = controller.getComponents(); 										//tab with all the components of the game pad (buttons, axes,...)
		if (comps.length == 0) {
		...
		System.out.println("x axis id : "+xAxisIdx);
		yAxisIdx = findCompIndex(comps, Component.Identifier.Axis.Y, "y-axis");
		System.out.println("y axis id : "+yAxisIdx);

		rxAxisIdx = findCompIndex(comps, Component.Identifier.Axis.RX, "rx-axis");
		System.out.println("rx axis id : "+rxAxisIdx);
		ryAxisIdx = findCompIndex(comps, Component.Identifier.Axis.RY, "ry-axis");
		System.out.println("ry axis id : "+ryAxisIdx);

		//delete the try/catch if your game pad has not enough axis and causes problems
		try {
			zAxisIdx = findCompIndex(comps, Component.Identifier.Axis.Z, "z-axis");
			System.out.println("z axis id : "+zAxisIdx);
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("la manette ne possède pas d'axe Z");
		}
	...
	public void poll() throws NoJoystickException{
	...
	public int getZRZStickDir() {
		...
		if ((rxAxisIdx == -1) || (ryAxisIdx == -1)) {
			System.out.println("(z,rz) axis data unavailable");
			return NONE;
		} else {
			return getCompassDir(rxAxisIdx, ryAxisIdx);
		}
	} // end of getXYStickDir()

	public int getZStickDir() {
		// return the (z,rz) analog stick compass direction

		if ((zAxisIdx == -1)) {
			System.out.println("(z) axis data unavailable");
			return NONE;
		} else {
			return getCompassDirZ(zAxisIdx);
		}
	}
	...
	// Return the axes as a single compass value
	{
		...
		//System.out.println("Coordonnées x = "+xCoord+" Coordonnées y = "+yCoord);
	...
	private int getCompassDirZ(int xA) {
		// Return the axes as a single compass value
		...
		if ((xc == -1))
			...
			System.out.println("Unknown (z): (" + xc + ")");
			return NONE;
		}
	}  // end of getCompassDir()
	...
	public boolean[] getButtons() //donne l'etat des boutons
	...
	public float getPosX(){
		return comps[0].getPollData()*100;
		
	}

	public float getPosY(){
		return comps[1].getPollData()*100;
	}

	public float getPosRX(){
		return comps[2].getPollData()*100;
	}

	public float getPosRY(){
		return comps[3].getPollData()*100;
	}

	public float getPosZ(){
		return comps[4].getPollData()*100;
	}
